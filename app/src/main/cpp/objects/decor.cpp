...
#include <objects/decor_config.hpp>
#include <random>
#include <algorithm>
#include <filesystem>


Decor::Decor(SDL_Renderer* renderer) {
    loadTextures(renderer);
}

RawTexture Decor::loadTexture(SDL_Renderer* renderer, const std::string& name) {
    std::string customPath = modding::joinPath(modding::getModdingDirectory(), "decor");
    ...
    if (name == "grass") return RawTexture(SDL_Incbin(SPRITE_DECOR_GRASS_PNG), renderer);
    if (name == "flower1") return RawTexture(SDL_Incbin(SPRITE_DECOR_FLOWER1_PNG), renderer);
    if (name == "flower2") return RawTexture(SDL_Incbin(SPRITE_DECOR_FLOWER2_PNG), renderer);
    if (name == "flower3") return RawTexture(SDL_Incbin(SPRITE_DECOR_FLOWER3_PNG), renderer);
    if (name == "flower4") return RawTexture(SDL_Incbin(SPRITE_DECOR_FLOWER4_PNG), renderer);
    if (name == "smallrock1") return RawTexture(SDL_Incbin(SPRITE_DECOR_SMALLROCK1_PNG), renderer);
    if (name == "smallrock2") return RawTexture(SDL_Incbin(SPRITE_DECOR_SMALLROCK2_PNG), renderer);
    if (name == "smallrock3") return RawTexture(SDL_Incbin(SPRITE_DECOR_SMALLROCK3_PNG), renderer);

    return RawTexture();
}

void Decor::loadTextures(SDL_Renderer* renderer) {
    auto assets = modding::loadDecorAssets();

    std::string configPath = modding::joinPath(modding::getModdingDirectory(), "decor.cfg");
...
void Decor::render(const SDL_Point& areaSize, const int& posY) {
    if (m_textures.empty()) {
        m_decorView.clear();
        m_nextGenY = posY + areaSize.y;
        m_lastPosY = posY;
        return;
    }
    ...
    while (posY + areaSize.y > m_nextGenY) {
        for (int i = 0; i < s_density; ++i) {
            ...
            m_decorView.push_back({ tex, { chosenX, m_nextGenY } });
        }
    ...
    m_decorView.remove_if([&areaSize](const auto& view) {
        SDL_Point size{};
